    ...
    db.executeSql(queryCountRespostasFutebol, [], function (res) {
        ...
        if (respostasFutebol>0){
            ...
            db.executeSql(queryCountRespostasCertasFutebol, [], function(result){
                ...
                var q1 = $('#mainDiv').html();
                q1 += "<div class='row'>";
                    q1 += "<div class='row' style='background-color: transparent; border-top: solid 1px #666666; border-left: solid 1px #666666;  box-shadow: 0 0 10px rgba(0, 0, 0, 0.4);'>";
                        q1 += result.rows.item(0).cp + " / " + res.rows.item(0).cp + " perguntas de Futebol certas (" + percent + "%)";
                    q1 += "</div>";
                q1 += "</div>";
                $('#mainDiv').html(q1);
            });    
    ...
    db.executeSql(queryCountRespostasFutebolIngles, [], function (res) {
        ...
        if (respostasFutebolIngles > 0){
            ...
            db.executeSql(queryCountRespostasCertasFutebolIngles, [], function(result){
                ...
                var q1 = $('#mainDiv').html();
                q1 += "<div class='row'>";
                    q1 += "<div class='large-12 medium-6 medium-centered small-12 columns'>";
                        q1 += result.rows.item(0).cp + " / " + res.rows.item(0).cp + " perguntas de Futebol em Inglês certas (" + percent + "%)";
                    q1 += "</div>";
                q1 += "</div>";
                $('#mainDiv').html(q1);
            });    
    ...
    db.executeSql(queryCountRespostasFutsal, [], function (res) {
        ...
        if (respostasFutsal >0){
            ...
            db.executeSql(queryCountRespostasCertasFutsal, [], function(result){
                ...
                q1 += "<div class='row'>";
                    q1 += "<div class='large-12 medium-6 medium-centered small-12 columns'>";
                        q1 += result.rows.item(0).cp + " / " + res.rows.item(0).cp + " perguntas de Futsal certas (" + percent + "%)";
                    q1 += "</div>";
                q1 += "</div>";
                $('#mainDiv').html(q1);
