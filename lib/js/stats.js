function HaPerguntasNaBD(db){
    var queryCountPerguntas = "select count(*) as cp from perguntas";
    db.executeSql(queryCountPerguntas, [], function(res){
        if ( res.rows.item(0).cp>0) return true;
    });
    var queryCountPerguntas = "select count(*) as cp from perguntasIngles";
    db.executeSql(queryCountPerguntas, [], function(res){
        if ( res.rows.item(0).cp>0) return true;
    });
    var queryCountPerguntas = "select count(*) as cp from perguntasFutsal";
    db.executeSql(queryCountPerguntas, [], function(res){
        if ( res.rows.item(0).cp>0) return true;
    });
    return false;
